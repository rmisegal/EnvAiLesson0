#!/usr/bin/env python3
"""
AI Environment Version Checker v3.0.8
...
Date: 2025-08-12
Version: 3.0.8 (Dynamic - shows highest system version)
"""
...
def get_highest_version_from_config():
...
def check_launch_method():
    ...
    if len(sys.argv) == 1 and not os.environ.get('CALLED_FROM_BAT'):
        print("=" * 70)
        print("⚠️  AI Environment Version Checker - Direct Launch Detected")
        print("=" * 70)
        print()
        print("🔍 You are running this Python script directly!")
        print()
        print("✅ RECOMMENDED way to check versions:")
        print("   D:\\AI_Environment\\check_versions.bat")
        print()
        print("ℹ️  CURRENT method (what you did):")
        print("   python src\\check_versions.py")
        print("   python D:\\AI_Environment\\src\\check_versions.py")
        print()
        print("📋 The check_versions.bat wrapper provides:")
        print("   1. Automatic Python detection (system or conda)")
        print("   2. Better error handling")
        print("   3. Consistent user experience")
        print()
        print("🔧 For best results, use: check_versions.bat")
        print("=" * 70)
        print()
        
        # Ask user if they want to continue
        try:
            response = input("Continue with direct Python execution? (y/N): ").strip().lower()
            if response not in ['y', 'yes']:
                print("\nExiting. Please use check_versions.bat for optimal experience.")
                sys.exit(0)
            else:
                print("\n⚠️  Continuing with direct Python execution...\n")
        except KeyboardInterrupt:
            print("\n\nExiting. Please use check_versions.bat for optimal experience.")
            sys.exit(0)

# Check launch method before proceeding
check_launch_method()
...
def get_highest_version_from_config(config):
    """Extract the highest version number from the configuration"""
    if not config:
    ...
    # Add versions from version_history
    if 'version_history' in config:
        for entry in config['version_history']:
            if 'version' in entry:
                versions.append(entry['version'])
    
    # Add versions from expected_versions
    if 'expected_versions' in config:
        for category in config['expected_versions'].values():
            if isinstance(category, dict):
                for file_info in category.values():
                    if isinstance(file_info, dict) and 'version' in file_info:
                        versions.append(file_info['version'])
    ...
    try:
        # Convert to version tuples for proper sorting
        version_tuples = []
        for v in versions:
            parts = v.split('.')
            version_tuples.append(tuple(int(x) for x in parts))
        
        version_tuples.sort(reverse=True)
        highest = '.'.join(str(x) for x in version_tuples[0])
        return highest
    except:
