...
def check_file_version(filepath: str, expected_version: str, search_pattern: str, description: str = ""):
...
def check_files_category(category_name: str, files_dict: dict, stats: dict) -> None:
    ...
    for filename, info in files_dict.items():
        ...
        success, message, status = check_file_version(
            filename, expected_version, search_pattern, description
        )
...
def print_summary(stats: dict) -> bool:
    ...
    if stats["correct"] == stats["total"] and stats["total"] > 0:
        percentage = 100.0
        print(
            f"{Colors.GREEN}[SUCCESS] All files have correct versions ({percentage:.0f}%){Colors.RESET}"
        )
        print(
            f"{Colors.GREEN}[INFO] Your AI Environment system is up to date!{Colors.RESET}"
        )
        return True
    else:
        percentage = (
        ...
        print(
            f"{Colors.YELLOW}[WARNING] Only {stats['correct']}/{stats['total']} files have correct versions ({percentage:.0f}%){Colors.RESET}"
        )
        print()

        if stats["wrong"] > 0:
            print(
                f"{Colors.YELLOW}[ACTION REQUIRED] {stats['wrong']} files have wrong versions{Colors.RESET}"
            )
        if stats["missing"] > 0:
            print(
                f"{Colors.RED}[ACTION REQUIRED] {stats['missing']} files are missing{Colors.RESET}"
            )

        print(
            f"{Colors.CYAN}[SOLUTION] Check version_config.json for expected versions{Colors.RESET}"
        )
        print(
            f"{Colors.CYAN}[SOLUTION] Update files as needed or download latest package{Colors.RESET}"
        )
        return False
